...
function contructTooltip(
	...
	const mainClass = /* plugin.settings.glassBg */ false
		? "fis-predictions-container-glass"
		: "fis-predictions-container";
						...
						if (category === IssueType.Spell) {
							...
							button.onclick = () => {
								const spellcheckDictionary: string[] =
									(plugin.app.vault as any).getConfig(
										"spellcheckDictionary"
									) || [];

								(plugin.app.vault as any).setConfig(
									"spellcheckDictionary",
									[
										...spellcheckDictionary,
										view.state.sliceDoc(
